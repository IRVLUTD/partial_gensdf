...
class OnlineObjectRenderer:
    ...
    def _load_object(self, path, scale, centroid):
        ...
        tmesh.vertices = tmesh.vertices / bbd        
    ...
    def change_object(self, path, scale, centroid):
        if self._current_context is not None:
            self._scene.remove_node(self._current_context['node'])

        if not self._caching:
            self._cache = {}
        self._current_context = self._load_object(path, scale, centroid)
    ...
    def change_and_render(self, cad_path, cad_scale, centroid, pose, render_pc=True):
        self.change_object(cad_path, cad_scale, centroid)
